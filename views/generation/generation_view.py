...
class GenerationView(View):
    ...
    data_sets = [("temp", "Temp"), ("cond", "Cond"), ("pH_acc", "pH_acc")]
    ...
    def post(self, request, dataset=None):
        ...
        if generate == "false":
            ...
            data_df = get_generated_data(data_set, len_ts=len_ts, nb_ts=nb_ts, num_hashtables=num_hashtables,
                                         nb_top=nb_top, hash_length_percentage=hash_length_percentage, min=min_, max=max_)

            generated_data = [data_df.iloc[:, i].values.tolist() for i in range(data_df.shape[1])]
            original_data = pd.read_csv(f"{folder}/{data_set}/original.txt")

            original = pd.read_csv(f"{folder}/{data_set}/original.txt").iloc[min_:max_, 0].values.flatten().tolist()
            return JsonResponse({
