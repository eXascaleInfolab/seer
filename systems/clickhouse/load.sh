...
sudo docker exec -i clickhouse-container clickhouse-client --format_csv_delimiter="," -q "INSERT INTO $table_name FORMAT CSV" < ../../datasets/$dataset.csv

end_time=$(date +%s.%N)
elapsed_time="$(echo "$end_time - $start_time" | bc)"

echo "computing compression"
compression="$(sh compression.sh $table_name | tail -n 1)"

echo "$table_name $compression ${elapsed_time}s"

echo "$table_name $compression ${elapsed_time}s" >> time_and_compression.txt
